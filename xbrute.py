...
extra = input('Input the extra argument (example: for https-post-form): ')
if extra == '':
    print('No supplied argument.')
else:
...
print('''
Method:
[1] - Random characters - Random length (1-25)
[2] - Random characters - Random customizable length
[3] - Integer bruteforce
[4] - Specify file
''')
while True:
    ...
    if method == '1':
        while True:
            password = ''.join(random.SystemRandom().choice(alphabet) for c in range(random.randint(1,25)))
            os.system('hydra ' + login_spec + ' ' + login + ' -p ' + password + ' -e nsr -u -v -V ' + port + success_file + protocol + '://' + server + extra)
    if method == '2':
        while True:
            method_2_min_len = input('Input minimum length: ')
            if method_2_min_len == '':
                print('You must supply a minimum length.')
            else:
                try:
                    int(method_2_min_len) / 1
                except ValueError:
                    print('An integer is required.')
                else:
                    break
        while True:
            method_2_max_len = input('Input maximum length: ')
            if method_2_max_len == '':
                print('You must supply a maximum length.')
            else:
                try:
                    int(method_2_max_len) / 1
                except ValueError:
                    print('An integer is required.')
                else:
                    break
        while True:
            password = ''.join(random.SystemRandom().choice(alphabet) for c in range(random.randint(int(method_2_min_len),int(method_2_max_len))))
            os.system('hydra ' + login_spec + ' ' + login + ' -p ' + password + ' -e nsr -u -v -V ' + port + success_file + protocol + '://' + server + extra)
    if method == '3':
        password = 0
        while True:
